...
from hvq.utils.util_functions import dir_check
from hvq.viterbi_utils.cte_viterbi import Viterbi
from hvq.viterbi_utils.cte_grammar import CTEGrammar
...
class Video(object):
    ...
    def features(self):
        ...
        if self._features is None:
            if opt.ext == 'npy':
                self._features = np.load(self.path) # .T
    ...
    def _adjust_embed_dim(self):
        try:
            self.n_frames = self._embedding.shape[0]
            assert len(self.gt) == self.n_frames
        except AssertionError:
            min_n = min(len(self.gt), self.n_frames)
            self.gt = self.gt[:min_n]
            self.n_frames = min_n
            self._embedding = self._embedding[:min_n]
            self._likelihood_grid = np.zeros((self.n_frames, self._K))
            self._valid_likelihood = np.zeros((self.n_frames, self._K), dtype=bool)
